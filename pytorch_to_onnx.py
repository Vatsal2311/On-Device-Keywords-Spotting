...
model.load_state_dict(torch.load("Weight/Pytorch_Pretrain/Conv2D_Voice_Model.pt", map_location=torch.device('cpu')))
model.eval()
dummy_input = torch.randn(1, 1,1, 16000)  # one sample, 10 features (same as the model's input size)
save_path = "Weight/Onnx"
if (os.path.isdir(save_path) == False):
    os.makedirs(save_path)
onnx_file = "Weight/Onnx/Conv2D_Voice_Model.onnx"
